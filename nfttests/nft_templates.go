	...
	template := `
		...
		transaction {
		  prepare(acct: AuthAccount) {
			...
			acct.link<&{NonFungibleToken.CollectionPublic}>(/public/%s, target: /storage/NFTCollection)
		  }
		}
	`

	return []byte(fmt.Sprintf(template, nftAddr, tokenContractName, tokenAddr, storageName, storageName))
}
