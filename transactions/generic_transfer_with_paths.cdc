...
transaction(to: Address, id: UInt64, senderPathIdentifier: String, receiverPathIdentifier: String) {
    ...
    prepare(signer: auth(BorrowValue) &Account) {
        ...
        let storagePath = StoragePath(identifier: senderPathIdentifier)
            ?? panic("Could not construct a storage path from the path identifier \(senderPathIdentifier)")

        // borrow a reference to the signer's NFT collection
        let withdrawRef = signer.storage.borrow<auth(NonFungibleToken.Withdraw) &{NonFungibleToken.Collection}>(
                from: storagePath
            ) ?? panic("The signer does not store a NonFungibleToken Collection object at the path \(storagePath). The signer must initialize their account with this collection first!")
    ...
    execute {
        let publicPath = PublicPath(identifier: receiverPathIdentifier)
            ?? panic("Could not construct a public path from the path identifier \(receiverPathIdentifier)")
        ...
        let receiverRef = recipient.capabilities.borrow<&{NonFungibleToken.Receiver}>(publicPath)
            ?? panic("The recipient \(to) does not have a NonFungibleToken Receiver at the path \(publicPath) that is capable of receiving an NFT. The recipient must initialize their account with this collection and receiver first!")
