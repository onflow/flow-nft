...
func main() {
	...
	color.Green("This will start overflow in embedded mode")
...
In our case we deployed contracts and created the users alice and bob.`)
	...
	color.Cyan(`o.Tx("setup_account", SignProposeAndPayAs("alice")).Print()`)
	color.Green("")
...
In overflow v1 all arguments are _named_ that is you mention them by their name and the value and not the order they appear in the transaction. If you use the wrong names and types then overflow will let you know with an terse appropritate error message`)
	...
	color.Cyan(`
  id :=o.Tx("mint_nft", SignProposeAndPayAsServiceAccount(),
		Arg("recipient", "alice"),
		...
		Print().GetIdFromEvent("Deposit", "id")
		`)
	...
	color.Cyan(`o.Script("get_nft_metadata", Arg("address", "alice"), Arg("id", id)).Print()`)

	pause()
	o.Script("get_nft_metadata", Arg("address", "alice"), Arg("id", id)).Print()
